...
class Supermarket {
  ...
  public void DFSrec(int source, int visitedNum, int totalTime) {
	  visited[source]=1;
	  if (visitedNum==K+1) {
		  minTime=Math.min(minTime, totalTime+distance[source][0]);
	  }
	  else {
		  for(int j=0;j<K;j++) {
			  if(visited[shoppingList[j]]==0) {
				  DFSrec(shoppingList[j],visitedNum+1,totalTime+distance[source][shoppingList[j]]);
			  }
		  }
	  }
	  visited[source]=0;
